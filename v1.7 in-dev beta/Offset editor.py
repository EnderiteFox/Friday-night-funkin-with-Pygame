        ...
        def __init__(self, name, characterNum):
            if name != "None":
                ...
                try:
                    ...
                    dontFlip = "False"
                try:
                    self.textureDirection = json.load(open(name + os.path.sep + "characterData.json"))["texture_direction"]
                except:
    ...
    while True:
        for events in event.get():
            if events.type == QUIT or (events.type == KEYDOWN and events.key == K_ESCAPE):
                return None
            if events.type == MOUSEBUTTONDOWN and not (display.Info().current_w - clickHitboxWidth < events.pos[0] < display.Info().current_w and 0 < events.pos[1] < clickHitboxHeight):
                if events.button == 1:
